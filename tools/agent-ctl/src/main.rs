...
fn make_examples_text(program_name: &str) -> String {
    ...
    let server_address = "vsock://3:1024";
...
- Check if the agent is running:
  ...
  $ {program} connect --server-address "{server_address}" --cmd Check

- Query the agent environment:

  $ {program} connect --server-address "{server_address}" --cmd GuestDetails

- List all available (built-in and Kata Agent API) commands:

  $ {program} connect --server-address "{server_address}" --cmd list
...
- Attempt to create 7 sandboxes, ignoring any errors:
  ...
  $ {program} connect --server-address "{server_address}" --repeat 7 --cmd CreateSandbox

- Query guest details forever:

  $ {program} connect --server-address "{server_address}" --repeat -1 --cmd GuestDetails

- Send a 'SIGUSR1' signal to a container process:

  $ {program} connect --server-address "{server_address}" --cmd 'SignalProcess signal=usr1 sid={sandbox_id} cid={container_id}'

- Create a sandbox with a single container, and then destroy everything:

  $ {program} connect --server-address "{server_address}" --cmd CreateSandbox
  $ {program} connect --server-address "{server_address}" --bundle-dir {bundle:?} --cmd CreateContainer
  $ {program} connect --server-address "{server_address}" --cmd DestroySandbox

- Create a Container using a custom configuration file:

  $ {program} connect --server-address "{server_address}" --bundle-dir {bundle:?} --cmd 'CreateContainer spec={config_file_uri}'
	"#,
        bundle = bundle,
